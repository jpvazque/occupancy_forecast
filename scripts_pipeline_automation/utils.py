...
def prepare_timeserie_df(df, pd_series=False, series_index=None, perform_scale_data=True):
    ...
    if pd_series:
        if perform_scale_data:
            site_sacaled_data = scale_data(df, 'OCCUPANCY_COUNT')
        ...
        transformed_data = pd.DataFrame({'ds':df_grouped_dates['DATES'], 
                                         'y':df_grouped_dates['OCCUPANCY_COUNT']})
        if perform_scale_data:
            transformed_data = scale_data(transformed_data)
...
def prepare_train_test_timeseries_gluon(df,
                                        ...
                                        start_day=None):
    if perform_scale_data:
        df = scale_data(df, scaled_column_name=data_column_name)
    ...
    return timeserie_gluon_train, timeserie_gluon_test
...
def hyperparameter_tuning(df, 
    ...
    for params in tqdm(all_params, desc="Tuning: %s"%site_name):
        ...
        df_cv = cross_validation(p, initial=initial, horizon=horizon)#, parallel="processes")
